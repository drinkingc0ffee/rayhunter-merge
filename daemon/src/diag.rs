...
pub async fn start_recording(
    ...
    log::info!("[DEBUG] start_recording handler entered");
    if state.config.debug_mode {
        log::warn!("[DEBUG] start_recording: server is in debug mode");
    ...
    match state
        ...
        .await {
        Ok(_) => {
            log::info!("[DEBUG] start_recording: StartRecording message sent");
            Ok((StatusCode::ACCEPTED, "ok".to_string()))
        },
        Err(e) => {
            log::error!("[DEBUG] start_recording: couldn't send start recording message: {e}");
            Err((StatusCode::INTERNAL_SERVER_ERROR, format!("couldn't send start recording message: {e}")))
        }
    }
}
