#!/bin/bash
# Script to deploy rayhunter-daemon and gps_jwt_pin to the device
...
GPS_JWT_PIN_BINARY="target/armv7-unknown-linux-musleabihf/debug/gps_jwt_pin"
DAEMON_DEST="/data/rayhunter/rayhunter-daemon"
GPS_JWT_PIN_DEST="/data/rayhunter/tools/gps_jwt_pin"
...
adb push "$DAEMON_BINARY" /tmp/
echo "Pushing gps_jwt_pin to /tmp..."
adb push "$GPS_JWT_PIN_BINARY" /tmp/

# Create destination directories using rootshell
echo "Creating destination directories..."
adb shell "rootshell mkdir -p /data/rayhunter/tools"
